...
class TestBangPrefixCommands:
    ...
    @pytest.mark.parametrize(
        ("typed", "normalized", "canonical", "args"),
        [
            ("!reset", "/reset", "reset", ""),
            ("!model deepseek-v4-flash", "/model deepseek-v4-flash", "model", "deepseek-v4-flash"),
            ("!stop", "/stop", "stop", ""),
        ],
    )
    @pytest.mark.asyncio
    async def test_nonempty_first_thread_context_keeps_command_pristine(
        self, adapter, typed, normalized, canonical, args,
    ):
        """Thread backfill must never turn a Slack command into agent text."""
        context = (
            "[Thread context — prior messages in this thread "
            "(not yet in conversation history):]\n"
            "[thread parent] U_OTHER: prior context\n"
            "[End of thread context]\n"
        )
        adapter._fetch_thread_context = AsyncMock(return_value=context)
        adapter._fetch_thread_parent_text = AsyncMock(return_value="prior context")

        await adapter._handle_slack_message(
            self._make_event(typed, thread_ts="1111111111.000001")
        )

        adapter.handle_message.assert_awaited_once()
        msg_event = adapter.handle_message.await_args.args[0]
        assert msg_event.text == normalized
        assert msg_event.text.startswith("/")
        assert context not in msg_event.text
        assert msg_event.channel_context == context
        assert msg_event.message_type == MessageType.COMMAND
        assert msg_event.is_command()
        assert msg_event.get_command() == canonical
        assert msg_event.get_command_args() == args
        adapter._fetch_thread_context.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_nonempty_first_thread_context_stays_separate_for_plain_text(
        self, adapter,
    ):
        """Ordinary thread backfill crosses the adapter/gateway seam once."""
        ...
        await adapter._handle_slack_message(
            self._make_event("ordinary follow-up", thread_ts="1111111111.000001")
        )

        msg_event = adapter.handle_message.await_args.args[0]
        assert msg_event.text == "ordinary follow-up"
        assert context not in msg_event.text
        assert msg_event.channel_context == context
        assert msg_event.message_type == MessageType.TEXT
        assert not msg_event.is_command()
        adapter._fetch_thread_context.assert_awaited_once()

        runner = object.__new__(GatewayRunner)
        runner.config = GatewayConfig()
        runner._session_key_for_source = lambda _source: "slack-thread-session"
        prepared = await runner._prepare_inbound_message_text(
            event=msg_event,
            source=msg_event.source,
            history=[],
            session_key="slack-thread-session",
        )
        assert prepared is not None
        assert prepared.count(context) == 1
        assert prepared.count("ordinary follow-up") == 1
        assert f"{context}\n\n[New message]\nordinary follow-up" in prepared

    @pytest.mark.parametrize(
        ("typed", "normalized", "canonical"),
        [
            ("/reset", "/reset", "reset"),
            ("!reset", "/reset", "reset"),
            ("/new", "/new", "new"),
    ...
    @pytest.mark.asyncio
    async def test_nonempty_first_thread_reset_reaches_gateway_reset_not_agent(
        self, adapter, monkeypatch, typed, normalized, canonical,
    ):
        """Exercise Slack normalization through the real gateway dispatcher."""
        from gateway.config import GatewayConfig
        from gateway.run import GatewayRunner
        ...
        runner._running_agents = {"slack-thread-session": MagicMock()}
        ...
        runner._handle_reset_command = AsyncMock(return_value="reset dispatched")
        runner._handle_message_with_agent = AsyncMock(
            side_effect=AssertionError("threaded reset leaked into agent prompt")
        )
        runner._run_agent = AsyncMock(
            side_effect=AssertionError("threaded reset constructed an agent")
        )
        monkeypatch.setattr("hermes_cli.plugins.invoke_hook", lambda *_a, **_k: [])
        monkeypatch.setattr("tools.slash_confirm.get_pending", lambda _key: None)
        ...
        routed_event = adapter.handle_message.await_args.args[0]
        assert routed_event.text == normalized
        assert routed_event.channel_context == context
        assert routed_event.get_command() == canonical
        runner._interrupt_and_clear_session.assert_awaited_once()
        runner._handle_reset_command.assert_awaited_once_with(routed_event)
        runner._handle_message_with_agent.assert_not_awaited()
        runner._run_agent.assert_not_awaited()

    def test_real_session_store_reset_rotates_and_clears_persisted_model_override(
        self, tmp_path, monkeypatch,
    ):
        """Reset rotation and override clearing survive a real SQLite reload."""
        import hermes_state
        from gateway.config import GatewayConfig
        ...
        try:
            ...
            assert store._db.db_path == state_path
            ...
            new_row = store._db.get_session(new_entry.session_id)
            ...
            store._db.close()
            reloaded = SessionStore(sessions_dir, config)
            assert reloaded._db is not None
            assert reloaded._db.db_path == state_path
            reloaded._ensure_loaded()
            persisted = reloaded._entries[session_key]
            assert persisted.session_id == new_entry.session_id
            assert persisted.is_fresh_reset is True
            assert reloaded.get_model_override(session_key) is None
        finally:
            if reloaded is not None and reloaded._db is not None:
                reloaded._db.close()
            if store._db is not None and store._db._conn is not None:
                store._db.close()
