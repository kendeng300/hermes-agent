...
| `cron/jobs.py::_get_due_jobs_locked` | Due selection can advance stale recurring rows before dispatch ownership exists. | Due selection is read-only; the claim owner performs successor plus claim atomically. |
...
| `cron/scheduler.py::run_one_job` | Calls existing one-shot `claim_dispatch`, then run/output/delivery/finalization. | Preserve its ABI and one-shot behavior; accept exact recurring/canary context from its caller. |
...
reread. `run_claimed(result: ChronosClaimResultV1, *, adapters, loop)` accepts
only `CLAIMED`, invokes the shared `run_one_job` body on its postimage, and uses
its optional context. `fire_due(job_id, *, fire_at, adapters, loop)` remains
the synchronous composition for non-HTTP callers. The Chronos override
implements the same signatures. HTTP handlers call `claim_due` synchronously
and create a task for `run_claimed`, so no task double-claims. An exception
after entering task creation leaves ACTIVE and returns 503; only a returned
task permits 202.
...
The canary invokes the existing full `run_one_job` ABI. It never calls
...
## 7. Pre-run and command result
The backwards-compatible ABI is `run_one_job(job, *, adapters=None, loop=None,
verbose=False, execution_context=None)`. Recurring/canary callers pass exact
`ExecutionContextV1`; `None` preserves the complete legacy one-shot/manual
path without fabricating a claim ID. For managed recurring/canary calls,
running, cleanup, quarantine, interruption, output, delivery, and finalization
keys all use `(jobs_file_realpath,job_id,mode,claim_id)`.
`run_one_job` selects exactly one finalization owner: recurring context invokes
`finalize_recurring_occurrence`, canary context invokes
`finalize_paused_job_canary`, and `None` invokes the unchanged legacy
`mark_job_run` path. A managed context never invokes `claim_dispatch` or a
second finalizer. Canary context also bypasses the legacy
`_clear_cleanup_fire_claim`; only its exact canary CAS owner may alter that
tagged claim. Recurring context cannot invoke any job-ID-only claim cleanup.
...
The route uses one closed stage fold. `pre_run` is
`NOT_CONFIGURED|SUCCESS|FAILED`; `agent` is
`NOT_APPLICABLE|NOT_RUN|SUCCESS|FAILED`; `output` is
`NOT_REQUESTED|SAVED|FAILED`; `delivery` is
`NOT_CONFIGURED|SUPPRESSED|DELIVERED|FAILED`; and finalization is
`APPLIED|FAILED`. Pre-run FAILED requires agent NOT_RUN. Agent NOT_APPLICABLE is
...
mask an earlier failure. If finalization fails, the command reports
`FINALIZATION_FAILED` while retaining every earlier stage value; otherwise the
first failure code in `PRE_RUN_FAILED`, `AGENT_FAILED`, `OUTPUT_FAILED`,
`DELIVERY_FAILED` order is reported, or `SUCCESS` when none failed. Invalid or
contradictory stage tuples are rejected and never finalized as success.
...
6. At startup capture profile, PID/start, executable, loaded scheduler path, checkout, and Git OID.
7. Read-only verification requires candidate ancestor of merge and running
   OID = deployed HEAD = freshly fetched merge, with stable PID/start/executable
   and scheduler path. No OID/state file is written.
...
- running identity rejects one-field PID/start/executable/path/OID changes.
