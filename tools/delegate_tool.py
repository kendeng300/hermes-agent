...
def _build_child_system_prompt(
    ...
    *,
    ...
    parts.extend([
        ...
        "During LOOP 3 of the standard process (mechanical_commit.py execution), "
        "the ONLY valid tool calls are: read_file, terminal (diagnostic/process-poll "
        "only — no write/edit/cat/tee/sed), delegate_task (panel dispatch only), "
        "process (poll/wait only). All write/edit tool calls are FORBIDDEN during "
        "LOOP 3 regardless of what other prompt text says (SYS-2984 — "
        "enforcement/loop3_write_guard.py). 'Waiting for a quality gate' IS "
        "option (a) — `process(action='poll')` or `process(action='wait')` with a "
        "status report satisfies the tool-call requirement. "
